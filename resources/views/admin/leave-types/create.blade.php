<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Jenis Cuti</h2>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-x-auto p-6">
                <form method="POST" action="{{ route('admin.leave-types.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_cuti" value="{{ old('nama_cuti') }}" class="w-full rounded-md border-gray-300 shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="jatah_hari_default" value="{{ old('jatah_hari_default', 0) }}" class="w-full rounded-md border-gray-300 shadow-sm">
                        @error('jatah_hari_default') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-4 flex items-center gap-2">
                        <input type="checkbox" name="perlu_lampiran" value="1" id="perlu_lampiran" {{ old('perlu_lampiran') ? 'checked' : '' }}>
                        <label for="perlu_lampiran" class="text-sm text-gray-700">Perlu lampiran (misal surat dokter)</label>
                    </div>

                    <div class="mb-6 flex items-center gap-2">
                        <input type="checkbox" name="mengurangi_saldo" value="1" id="mengurangi_saldo" {{ old('mengurangi_saldo') ? 'checked' : '' }}>
