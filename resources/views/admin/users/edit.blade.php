            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-x-auto p-6">
                ...
                <form method="POST" action="{{ route('admin.users.update', $user) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded-md border-gray-300 shadow-sm">
                        @error('name') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                        <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded-md border-gray-300 shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nip" value="{{ old('nip', $user->nip) }}" class="w-full rounded-md border-gray-300 shadow-sm">
                        @error('nip') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Jabatan</label>
                        <input type="text" name="jabatan" value="{{ old('jabatan', $user->jabatan) }}" class="w-full rounded-md border-gray-300 shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="role" class="w-full rounded-md border-gray-300 shadow-sm">
                            ...
                            <option value="atasan" {{ old('role', $user->role) === 'atasan' ? 'selected' : '' }}>Kepala Balai</option>
